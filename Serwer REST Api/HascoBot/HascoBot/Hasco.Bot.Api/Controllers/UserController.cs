...
namespace Hasco.Bot.Api.Controllers
{
    ...
    public class UsersController : ApiBaseController
    {
        ...
        [HttpPost("giveaway/start")]
        public async Task<IActionResult> StartGiveaway()
        {
            var user = await _userService.GetById(UserId);
            await _twitchBot.StartGiveaway(user.ChannelName);

            return Ok();
        }

        [HttpPost("giveaway/winner")]
        public async Task<IActionResult> DrawTheWinner()
        {
            var user = await _userService.GetById(UserId);
            await _twitchBot.DrawTheWinner(user.ChannelName);

            return Ok();
        }

        [HttpPost("phrase/add")]
        public async Task<IActionResult> AddPrase([FromBody]Phrase phrase)
        {
            var user = await _userService.GetById(UserId);
            await _twitchBot.AddPhrase(user.ChannelName, phrase.Word);

            return Ok();
        }

        [HttpPost("phrase/remove")]
        public async Task<IActionResult> RemovePrase([FromBody]Phrase phrase)
        {
            var user = await _userService.GetById(UserId);
            await _twitchBot.RemovePhrase(user.ChannelName, phrase.Word);
